...
def measure_allZones_OES(wl, zoneList, measuredElementList, OESmaxMins, savedir, savedate, OESdataDict, darkChannel):
	...
	zoneList = ['1B', '2B',]
	MPToZoneMap = {'1B':1,'2B':2} #dict mapping zones measured to multiplexer channels
	...
	for each in range(len(zoneList)):
		OESdataDict[zoneList[each]]['DT'], rawOESspectrum = measure_OES_spectrum(MPToZoneMap[zoneList[each]], darkInt)
		for element in measuredElementList:
...
if __name__ == '__main__':
	...
	savedir = 'C:/OESdata/' + 'PC and Na burn off ' + savedate + '/'
