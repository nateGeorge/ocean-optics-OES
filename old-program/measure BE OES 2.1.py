...
def measure_allZones_OES(wl, zoneList, measuredElementList, OESmaxMins, savedir, savedate, OESdataDict, darkChannel):
	...
	for each in range(len(zoneList)):
		indexToZoneMap = {'1B':1,'2B':2,'3B':15,'4B':16} #dict mapping zones measured to multiplexer channels
		OESdataDict[zoneList[each]]['DT'], rawOESspectrum = measure_OES_spectrum(indexToZoneMap[zoneList[each]], darkInt)
		for element in measuredElementList:
...
if __name__ == '__main__':
	...
	while True:
		...
		if plotCounter == 0:
			subprocess.Popen(['python','Y:/Nate/new MC02 OES program/backup from MC02 computer/plotOES-BE 2.0.pyw'])
			plotCounter += 1
		#f.clf()
