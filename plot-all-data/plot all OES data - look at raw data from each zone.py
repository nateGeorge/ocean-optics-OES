...
for zone in zoneList:
	...
	for each in [10]:#range(trimmedZoneOESrawspectra[zone].shape[0]):
		plt.plot(wl,trimmedZoneOESrawspectra[zone][each,:])
		#maximaIndices = signal.find_peaks_cwt(np.array(trimmedZoneOESrawspectra[zone][each,:],dtype='float64'), np.arange(2,8), min_snr = 5, noise_perc = 10)
		#plt.scatter([wl[index] for index in maximaIndices],[trimmedZoneOESrawspectra[zone][each,index] for index in maximaIndices],c='r')
		...
		plt.show()#plt.savefig('DW ' + str(round(plotDWZone[zone][each],1)) + '.png',facecolor = 'w', dpi = 600)
		plt.clf()
	os.chdir('../')
